...
fn test_slice_stream_input() {
    ...
    // setup command without specific file
    cmd.arg("slice")
       .arg("--min-length")
       .arg(min_length.to_string())
       .arg("--max-length")
       .arg(max_length.to_string())
       .write_stdin(input_data)  // Write directly to stdin
       .assert()
       .success();
    ...
    assert!(!fragments.is_empty(), "There should be at least one fragment");
